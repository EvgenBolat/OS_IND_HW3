...
void HandleTCPClient(
    int clntSocket, pid_t pid, int serversCount,
    struct sockaddr_in
        *echoListnerServAddrs); /* TCP client handling function */
...
void makeServerLog(char *sending) {
  ...
  for (int serverNum = 0; serverNum < serversCount; serverNum++) {
    ...
    if (connect(lstnsock, (struct sockaddr *)&echoListnerServAddrs[serverNum],
                sizeof(echoListnerServAddrs[serverNum])) < 0) {
...
int main(int argc, char *argv[]) {
  ...
  for (;;) /* Run forever */
  {
    ...
    HandleTCPClient(clntSock, pid, serversCount, echoListnerServAddrs);
    makeServerLog("Покупатель ушёл");
    printf("Продавец %d: Покупатель ушел\n", pid);
    putchar('\n');
    printf("Продавец %d: Вы хотите изменить состав серверов-слушателей?(Введите y или n)\n", pid);
    if (getchar() == 'y') {
      printf("Продавец %d: Введите количество серверов-слушателей, которые вы хотите "
             "использовать(от 1 до 10): ", pid);
      scanf("%d", &serversCount);
      for (int i = 0; i < serversCount; i++) {
        printf("Продавец %d: Введите айпи и порт сервера-слушателя: ", pid);
        scanf("%s", new_server_string);
